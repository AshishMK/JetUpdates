...
package com.demo.jetupdates.feature.chat.impl
...
import com.demo.jetupdates.core.ui.MessagePreviewParameterData
...
class ChatScreenTest {
                ...
                AppBackground {
                    ChatScreen(
                        feedState = ConversationUiState(
                            initialMessages = MessagePreviewParameterData.initialMessages,
                        ),
                        onShowSnackbar = { _, _ -> false },
                        ask = {},
    ...
    fun feed_shows_scroll_on_reverseLayout() {
        composeTestRule
            .onNodeWithText(
                MessagePreviewParameterData.initialMessages.first().content,
                substring = true,
            )
            .assertExists()
        ...
        composeTestRule.onNode(hasScrollToNodeAction())
            .performScrollToNode(
                hasText(
                    MessagePreviewParameterData.initialMessages.last().content,
                    substring = true,
                ),
            )
    ...
    fun feed_only_distinct_author_messages_item_has_avatar() {
        assertNotEquals(
            MessagePreviewParameterData.initialMessages[1].author,
            MessagePreviewParameterData.initialMessages[2].author,
        )
        // or
        assertTrue(
            MessagePreviewParameterData.initialMessages[1].author.equals(
                MessagePreviewParameterData.initialMessages[0].author.uppercase(),
                true,
            ),
        )
        ...
        MatcherAssert.assertThat(
            MessagePreviewParameterData.initialMessages[1].author,
            IsEqualIgnoringCase.equalToIgnoringCase(MessagePreviewParameterData.initialMessages[0].author.uppercase()),
        )
        composeTestRule
            .onNodeWithContentDescription(
                MessagePreviewParameterData.initialMessages[1].content,
            )
            .assertExists().assertIsDisplayed()

        TestCase.assertEquals(
            MessagePreviewParameterData.initialMessages[1].author,
            MessagePreviewParameterData.initialMessages[0].author,
        )

        composeTestRule
            .onNodeWithContentDescription(
                MessagePreviewParameterData.initialMessages[0].content,
            )
            .assertDoesNotExist()
    }
    ...
    private fun findJumpToBottom() =
        composeTestRule.onNodeWithText(
            composeTestRule.activity.getString(R.string.core_ui_jumpBottom),
    ...
    private fun openEmojiSelector() =
        composeTestRule
            .onNodeWithTag(
                composeTestRule.activity.getString(R.string.core_ui_emoji_selector_bt_desc),
            )
            .performClick()
}
