...
import com.demo.jetupdates.feature.chat.api.R
import com.demo.jetupdates.feature.chat.impl.ui.ChatFlexBoxLayout
...
internal fun ChatScreen(
    ...
    val authorMe = stringResource(R.string.feature_chat_api_author_me)
    val timeNow = stringResource(R.string.feature_chat_api_now)
    var test by rememberSaveable { mutableStateOf(false) }
    var hideKeyBoard by rememberSaveable { mutableStateOf(false) }

    BackHandler(enabled = onBack != null || test) {
        if (test) {
            hideKeyBoard = true
            test = false
        } else {
...
fun Messages(
    ...
    Box(modifier = modifier) {
        val authorMe = stringResource(id = R.string.feature_chat_api_author_me)
        LazyColumn(
            reverseLayout = true,
            state = scrollState,
            modifier = Modifier
