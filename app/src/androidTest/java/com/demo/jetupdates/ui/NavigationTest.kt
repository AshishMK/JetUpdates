...
class NavigationTest {
    ...
    private val sampleCategory = "Headlines"
...
*/
    ...
    fun navigationBar_reselectTab_keepsState() {
        composeTestRule.apply {
            // GIVEN the user follows a category
            onNodeWithText(sampleCategory).performClick()
            // WHEN the user taps the Store navigation bar item
...
//        // GIVEN the user is on the Categories destination and scrolls
//        // and navigates to the Category Detail destination
//        // WHEN the user taps the Categories navigation bar item
//        // THEN the Categories destination shows in the same scrolled state
//    }
    ...
    fun navigationBar_multipleBackStackInterests() {
        composeTestRule.apply {
            ...
            onNodeWithTag("interests:categories").performScrollToNode(hasText(category.name))
    ...
    @Test
    fun navigatingToCategoryFromStore_showsCategoryDetails() {
        composeTestRule.apply {
            ...
            // Get the shop item feed and scroll to the shop item
            // Note: Possible flakiness. If the content of the shop item is long then the category
            ...
            onNodeWithTag("store:feed")
                ...
                .fetchSemanticsNode()
                .apply {
                    val shopItemCardNode = onNodeWithTag("shopItemCard:${shopItem.id}")
                        .fetchSemanticsNode()
                    config[ScrollBy].action?.invoke(
                        ...
                        (shopItemCardNode.size.height - size.height).coerceAtLeast(0).toFloat(),
                    )
                }
